...
import { getAllBlogCategories, createBlogCategory, canCreateCategory, getCategoryCount } from '@/lib/db/queries/blog-categories';
...
export const GET: APIRoute = async ({ request, locals }) => {
  try {
    ...
    const categories = await getAllBlogCategories(db, onlyActive);
...
export const POST: APIRoute = async ({ request, locals }) => {
  try {
    ...
    const category = await createBlogCategory(db, validationResult.data);
